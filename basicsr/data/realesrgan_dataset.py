...
from basicsr.data.paired_image_dataset import TiffPairedImageDataset, read_tiff, center_crop
...
@DATASET_REGISTRY.register(suffix="basicsr")
class TiffRealESRGANDataset(TiffPairedImageDataset):
    """Dataset used for Real-ESRGAN model:
    Real-ESRGAN: Training Real-World Blind Super-Resolution with Pure Synthetic Data.
    ...
    def __init__(self, opt):
        super(TiffRealESRGANDataset, self).__init__(opt)
        self.opt = opt
    ...
    def __getitem__(self, index):
        ...
        img_gt, meta_gt, selected_bands = read_tiff(gt_path, bands, self.min_values_gt, self.max_values_gt)
        img_lq, _, _ = read_tiff(lq_path, selected_bands, self.min_values_lq, self.max_values_lq)
